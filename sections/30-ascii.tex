...
\begin{frame}{\currenttitle}
% Let's actually write an ASCII encoder. This shouldn't be too difficult.
% The conversion of characters to code points and vice versa is typically done at the operating
% system level.
%
% In modern programming languages, the character data type is typically a primitive like int or
% float that is a number.
%
% For demonstration's sake, we're going to simply pretend that the char data type in this Rust
% code is not already a numerical value.
  ...
  \lstinputlisting[language=Rust]{\subdir/32-encoder.rs}
\end{frame}

% ---------------------------
\renewcommand{\currenttitle}{The drawbacks}
\begin{frame}{\currenttitle}
...
% What if we want to write kanji? How do we encode and store that?
% What about e with an acute accent to write words like résumé, or write French?
  What about all the other characters out there?
\end{frame}
