...
\newcommand{\arabicexample}{%
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \framebox{\texttt{backspace}\footnotemark{}} \\[3em]
...
%
% Here's part of our header file, where we'll define the function signatures and some
% constants.
%
% This is probably not a good practice, but we're going to alias unsigned char to a type
% called
...
% be positive.
% Thus, a byte, which we've defined as unsigned char, will simply be an 8-bit integer,
% representing an actual byte.
% Ints are 32-bit integers, so unsigned int denotes a positive 32-bit integer.
% Remember that a codepoint can fit in 21 bits, so a 32-bit integer will be large enough to
% represent every possible codepoint.
%
...
\begin{frame}[fragile]{\currenttitle}
% So that's our UTF-8 decoder.
% I encourage you, this week, to try writing the reverse: a UTF-8 encoder.
%
% That is, write a function that takes in a list of Unicode codepoints and returns a list of
% UTF-8-encoded byte values.
% We'll have a form you can submit your solution to if you'd like.
  \vspace*{1em}
...
\section{\sectiontitle}
% Now that we have a little better of an idea of how modern computers handle text, how can
% we make sure the programs we write are conscious of these complexities?

% ---------------------------
\renewcommand{\currenttitle}{Understand your language}
\begin{frame}{\currenttitle}
% The first step is to understand how the language you're using handles text.
%
% Hopefully, after everything we've discussed, you noticed that there are a lot of
% considerations to be had.
% We talked more extensively about UTF-8, but that doesn't mean everything is necessarily
% UTF-8.
% That's why it's important to understand how text data is stored.
%
...
% Are strings handled as bytes?
%
% As you explore different languages, you'll find that some handle strings natively as UTF-8,
% while some have no built-in Unicode support.
% For the latter, it may be wise to use an external library that handles encoding, decoding,
% and more advanced operations for you.
%
% In most cases, languages will have sufficient functionality for most basic operations.
% However, once you start doing comparisons, regular expressions and more advanced operations
% that require awareness of character categories and things like that, you may need an
% external library that is Unicode-aware.
%
% Let's take a look at some of these complications.
  Consider:
...
\begin{frame}[fragile]{\currenttitle}
% We cannot index strings to get characters.
...
\begin{frame}[fragile]{\currenttitle}
% For similar reasons, we need to conscious of what's going on when we iterate through strings.
%
% Let's say we have this kind of psuedo-code: a loop through some UTF-8-encoded string and we
% print each element.
% The question is: what's being printed?
%
% Are we printing each byte? Each codepoint? Each grapheme cluster?
% Of course, this is typically dependent on how the specific programming language handles text,
% which brings us back to the first point: know your language.
%
% If the string is encoded in UTF-8 and we want to print out each codepoint, we would need to
% do a similar process as with the UTF-8 decode we implemented earlier.
%
% If we want to print out grapheme clusters, which is what users typically perceive as
% characters in many languages, the code must be aware of which codepoints for clusters.
% This is all non-trivial and it would be best to rely on a Unicode-aware library.
...
\begin{frame}[fragile]{\currenttitle}
% Similarly to iterating grapheme clusters, many other operations require Unicode awareness.
%
% Let's look at a search and replace example.
% Suppose, that for any arbitrary string, we want to remove the numbers.
%
% Any particular writing script may have it's own number characters.
% We can't simply pass in the number characters we're all familiar with to match every
% number.
% Thus, this operation is non-trivial and requires a library aware of Unicode properties.
% This is why writing a Unicode-compliant regular expression library is a non-trivial task.
%
% Here's how we would do such an operation in standards-compliant modern JavaScript.
% This is a pretty mundane example since I was lazy.
% The 2 after the and is known as the Fullwidth Digit Two, which according to the Unicode
% standard is a decimal number, but not the same as the ASCII-compatible 2 we're use
% regularly.
...
\begin{frame}[fragile]{\currenttitle}
% As we mentioned before with the Amélie example, Unicode allows many equivalent
% representations of the same strings.
%
% This can pose complications when doing things like comparisons.
% Two strings may have different byte representations, but are considered canonically
% equivalent.
% If your comparison wants to reflect this (which in many cases it would make sense to)
% this is suddenly a non-trivial operation.
%
% Though some might consider these as limitations, I think it's a benefit.
...
\begin{frame}[fragile]{\currenttitle}
% We've kind of been talking about text encoding completely in the context of runtime memory
% and things like that, but of course encoding applies to pretty much everything.
%
% You've probably opened a file before and just seen a bunch of gibberish.
% It's possible that the file was simply a binary, but it's also possible that the file is in
% a different encoding than what your editor or program expected.
%
% On Linux systems, files are UTF-8 by default.
% On Windows systems, the default encoding is UTF-16.
% This is something to keep in mind encodings when you're dealing with files.
% 
% Most programming languages allow you to specify the encoding when performing file I/O.
